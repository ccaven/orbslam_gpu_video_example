    ...
    fn compatible_list_by_resolution(
        ...
        for camera_format in mf_camera_format_list {
            ...
            match resolution_map.get_mut(&camera_format.resolution()) {
                ...
                None => {
                    if let Some(mut old_frame_rates) = resolution_map
                        .insert(camera_format.resolution(), vec![camera_format.frame_rate()])
                    {
                        old_frame_rates.push(camera_format.frame_rate());
                        resolution_map.insert(camera_format.resolution(), old_frame_rates);
                    }
                }
            }
        }
